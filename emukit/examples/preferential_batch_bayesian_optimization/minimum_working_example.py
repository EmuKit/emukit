...
noise_level = 0.05
objective = evalset.test_funcs.Noisifier(
    objective, "add", noise_level
)  # add normally distributed noise with std 'noise_level'

# Generate a kernel for the GP
kernel = GPy.kern.RBF(
    input_dim=len(objective.bounds), ARD=True, variance=1.0, lengthscale=[0.5] * len(objective.bounds)
)
