...
def convert_xy_lists_to_arrays(x_list: List, y_list: List) -> Tuple[np.ndarray, np.ndarray]:
    ...
    :param x_list: A list of (n_points x n_dims) numpy arrays ordered from lowest to highest
                   fidelity.
    :param y_list: A list of (n_points x n_outputs) numpy arrays representing the outputs
                   ordered from lowest to highest fidelity.
    :return: Tuple of (x_array, y_array) where x_array contains all inputs across all fidelities
             with the fidelity index appended as the last column and y_array contains all outputs
