...
from emukit.bayesian_optimization.acquisitions import QExpectedImprovement
...
def test_acquisition_qei():
    ...
    # Compute acquisition:
    qei_analytic, _, _ = QExpectedImprovement.get_acquisition(mu, Sigma, current_minimum)
    qei_analytic_fast, _, _ = QExpectedImprovement.get_acquisition(mu, Sigma, current_minimum,
                                                                   fast_compute=True, eps=1e-3)
...
def test_acquisition_gradient_qei():
    ...
    _check_grad(QExpectedImprovement(model), TOL_GRAD, x0)
    _check_grad(QExpectedImprovement(model, fast_compute=True, eps=1e-3), TOL_GRAD_FAST, x0)
